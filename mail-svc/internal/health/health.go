// Package health serves Herald's own liveness endpoint (compose-healthcheck
// parity with Kiln's and Aegis's /healthz) and, when wired, the Prometheus
// /metrics endpoint — same internal mux, port unpublished in compose.
package health
...
func Handler(metrics http.Handler) http.Handler {
	...
	mux.HandleFunc(healthzPath, func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = io.WriteString(w, `{"status":"ok","service":"herald"}`+"\n")
	})
	if metrics != nil {
