...
// EventTypeProcessingRequested is the only event type the image service handles in v1.
...
func (h *Handler) Handle(ctx context.Context, raw string) (Outcome, error) {
	...
	if env.EventType != EventTypeProcessingRequested {
		// Forward-compat: the stream may carry new event types before the image service
