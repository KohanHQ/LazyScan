// Package store is the mail service's Postgres access layer: the worker bookkeeping
// tables from LazyScan migration 023 (processed-events dedup, durable failure
// rows). The mail service is a pure email projector — it never reads or writes the auth
// tables themselves.
package store
...
func (s *Store) IsProcessed(ctx context.Context, eventID string) (bool, error) {
	...
	err := s.pool.QueryRow(ctx,
		`SELECT 1 FROM mail_processed_events WHERE event_id = $1`,
		eventID,
	).Scan(&one)
...
func (s *Store) MarkProcessed(ctx context.Context, eventID, eventType string) error {
	_, err := s.pool.Exec(ctx,
		`INSERT INTO mail_processed_events (event_id, event_type)
		 VALUES ($1, $2)
...
// RecordFailure inserts a durable failure row (mail_failures), keeping the
// raw stream message for poison diagnosis.
func (s *Store) RecordFailure(ctx context.Context, f Failure) error {
	_, err := s.pool.Exec(ctx,
		`INSERT INTO mail_failures (event_id, user_id, error_message, raw_event, retryable)
		 VALUES ($1, $2, $3, $4, $5)`,
