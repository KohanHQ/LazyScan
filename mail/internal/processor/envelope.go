...
// the IDs-only envelope rule: the mail service is a pure email projector with nothing
...
func parseEnvelope(raw string) (Envelope, Payload, error) {
	...
	// Reject, never unwrap. No payload preview in the error (the image service includes
	// one): these errors reach logs and mail_failures.error_message, and
