// Package consumer is the mail service's Redis Streams transport: consumer-group reads,
...
func New(rdb *redis.Client, h *processor.Handler, claimIdle time.Duration, maxDeliveries, concurrency int, log *slog.Logger) *Consumer {
	...
	return &Consumer{
		rdb:           rdb,
		handler:       h,
		name:          fmt.Sprintf("mail-%s-%d", hostname, os.Getpid()),
		claimIdle:     claimIdle,
...
// read/claim batch are handled concurrently (MAIL_SVC_CONSUMER_CONCURRENCY);
...
// flight (main's drain-then-close shutdown ordering depends on that). A nil
// counts map means deliveries are unknown — dispatch treats 0 as "never
// poison".
