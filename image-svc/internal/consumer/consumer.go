// Package consumer is Kiln's Redis Streams transport: consumer-group reads,
...
func New(rdb *redis.Client, h *processor.Handler, claimIdle time.Duration, maxDeliveries, concurrency int, log *slog.Logger) *Consumer {
	...
	return &Consumer{
		rdb:           rdb,
		handler:       h,
		name:          fmt.Sprintf("kiln-%s-%d", hostname, os.Getpid()),
		claimIdle:     claimIdle,
...
// read/claim batch are handled concurrently (KILN_CONSUMER_CONCURRENCY);
